...
/**
 * Pantalla "Historial de auditoría" (solo Administrador). Mismo patrón
 * server-side que src/app/usuarios/page.tsx y src/app/settings/page.tsx: el
 * chequeo de perfil se hace acá, antes de mandar nada al navegador, usando
 * esAdministrador() (que también acepta "Administradora", ver
 * src/lib/permissions.ts) en vez de comparar el string a mano. El middleware
 * (src/middleware.ts) ya garantiza que solo se llega hasta acá con una
 * sesión real — el `redirect('/login')` es un respaldo.
 */
      ...
      <header className="chat-header-on-brand sticky top-0 z-20 border-b border-[var(--chat-border-strong)] bg-[var(--chat-header)] px-4 py-3 safe-area-top">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <div className="flex min-w-0 items-center gap-2">
              <ScrollText className="size-4 flex-shrink-0 text-[var(--chat-presence)]" />
              <h1 className="truncate text-base font-semibold text-foreground">Historial de auditoría</h1>
            </div>
          </div>
